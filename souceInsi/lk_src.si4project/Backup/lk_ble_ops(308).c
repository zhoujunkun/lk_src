...
void ble_heat_gear(rxFrame_t *frame)
{
	...
	frameHandle.= frame->buf[0];
	frameHandle.masterId = frame->buf[1];
}

//按摩时间
void ble_massage_time(rxFrame_t *frame)
{
   frameHandle.massageRuntime = frame->buf[0];
...
void ble_init(void)
{
   ...
   frameHandle.frameAnasys = ble_anysys_queue;
   frameHandle.heatGears = 0;
   frameHandle.massageGears = 0;
}
