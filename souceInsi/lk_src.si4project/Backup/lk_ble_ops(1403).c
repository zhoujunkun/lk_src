...
void ble_heat_gear(rxFrame_t *frame)
{
	...
	if(frameHandle.heatGears >=1) frameHandle.bleMassage.ifOver = false;
}

//按摩时间
void ble_massage_time(rxFrame_t *frame)
{
   frameHandle.bleMassage.runtime = frame->buf[0];
   frameHandle.masterId = (masterId_enum_t)(frame->buf[1]);
...
{
	frameHandle.bleMassage.gears = frame->buf[0];
	if(frameHandle.bleMassage.gears >=1) frameHandle.bleMassage.ifOver = false;
	frameHandle.masterId = (masterId_enum_t)(frame->buf[1]);
...
{
	frameHandle.bleMassage.mode = (massageMode_enum_t)(frame->buf[0]);
	frameHandle.masterId = (masterId_enum_t)(frame->buf[1]);
}
...
{
	frameHandle.bleMassage.ifOver = true;
	frameHandle.masterId = (masterId_enum_t)(frame->buf[1]);
...
int  ble_anysys_queue(SqQueue *bleQueue)
{
   ...
   int erroType=0;
	if(frameHandle.queueInvoke->getLength(bleQueue)>=1)
	{
       ...
       erroType = frameHandle.ble->myanasys(data,myBlefunc);
	}
 ...
 * @return      None
 */
uint8_t  bleName[] = {"AT+NAME=SKG_4045\r\n"};
void ble_init(bleSend sendCallBack)
{
   ...
   sendCallBack(bleName,sizeof(bleName));
...
void ble_massageRuntime_ack(void)
{
  ...
  sendArray[4] = frameHandle.bleMassage.runtime; //  
  sendArray[7]= check_sum(sendArray,7);
  sendFrame(sendArray,8);
}
