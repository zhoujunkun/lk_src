  ...
  uint8_t *pdata =NULL;
   ...
   daya	=devstatu->product;
	 devstatu->product =(daya>>8)|((daya&0xFF)<<8);  //大小端问题
	  daya = devstatu->runTime;
	 devstatu->runTime = (daya>>8)|((daya&0xFF)<<8);
   for(int i=0;i<sizeof(bleDeviceStatu_t);i++)
   {	 
       bleSendHeartFrame.sendBuf[4+i] = *(pdata++);
   }
   bleSendHeartFrame.frame.checkSum = check_sum((uint8_t*)(&bleSendHeartFrame),sizeof(bleHeartFrame_t));
   sendCallBack(bleSendHeartFrame.sendBuf,bleSendHeartFrame.frame.size);
}
